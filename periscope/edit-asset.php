...
if (isset($_POST["submit"])){
    ...
    if(empty($errors)) {
        ...
        if(isset($_GET["eaid"])) {
            ...
            if($get_class($new_asset) == "Standard") {
                $new_asset->standardListItem = (int)$db->mysql_prep($_POST["std"]);
            }
    ...
    <ul id = "asset-list">
        <?php
            ...
            foreach($assets as $asset) {
                ...
                echo "<a class='button' href='" . substr($_SERVER["REQUEST_URI"],0,strpos($_SERVER["REQUEST_URI"],"&")) . "&a={$_GET['a']}&eaid=" . $asset->id . "'>Edit</a> " .
                 "<a class='button' href='". substr($_SERVER["REQUEST_URI"],0,strpos($_SERVER["REQUEST_URI"],"&")) . "&a={$_GET['a']}&daid=" . $asset->id . "'>Remove</a>" . 
                  "</li>";
...
//        Selector script for Standards
        if($_GET["a"] === "std"){
           require_once("standard-form.php");
        } 
    ?>
    <input size="30" id="asset-text" type="text" name="asset-text"><br/>
    <?php echo "{$asset_title} Text";?></input><br>
    <?php
        ...
        if($_GET["a"] === "ass") {
            ...
            echo "<select class='button' id=\"assessment-type\" name=\"assessment-type\">";
